...
plot_params.Village <- function(village) {
  ...
  if (village$num_donorcov > 0) {
    ...
    model <- stringr::str_remove_all(
      village$model,
      "\\s*\\+?\\s*\\w*:treatment_scaled|treatment_scaled:\\w*|treatment_scaled"
    )

    # Clean up formula formatting
    model <- stringr::str_replace_all(model, "\\+\\s*\\+", "+")
    model <- stringr::str_remove_all(model, "\\+\\s*$")
    model <- stringr::str_remove_all(model, "^~\\s*\\+?")
  ...
  # Plot betas
  suppressWarnings(suppressMessages(plot_eta(village, lowci, highci, df_all, df_donor)))
  ...
  if (length(village$donorcov) > 0) {
    ...
    df_dcov <- df_dcov |>
      dplyr::group_by(cov.id, cov, chain) |>
...
plot_ppck.Village <- function(village) {
  ...
  if (isFALSE(village$ppck)) {
    stop(
      "No posterior predictive checks (ppck) performed. To include ppcks set ppck = TRUE when running model"
    )
  }

  df_ppck <- rstan::extract(
    village$fit,
    pars = unique(village$summary_ppck$paramname),
    permuted = FALSE
  ) |>
    as.data.frame()

  df_ppck <- tidyr::pivot_longer(
    df_ppck,
    cols = 1:ncol(df_ppck)
  )

  df_ppck$sample <- stringr::str_remove(df_ppck$name, ".*,") |>
    stringr::str_remove("\\]") |>
    as.factor()

  df_ppck$donorid <- stringr::str_remove(df_ppck$name, ".*\\[") |>
    stringr::str_remove(",.*") |>
    as.numeric()

  df_ppck$name <- NULL

  df_donor <- village$data_noT0[, c("donor", "donorid")] |>
    dplyr::distinct(donor, donorid)

  df_ppck <- merge(df_ppck, df_donor, by.x = "donorid")

  if (length(village$treatcol) == 1) {
    df_time <- village$data[
      village$data$time != 0,
      c("sample", "time", "replicate", village$treatcol)
    ] |>
      dplyr::distinct()
  } else {
    df_time <- village$data[
      village$data$time != 0,
      c("sample", "time", "replicate")
    ] |>
      dplyr::distinct()
  }
  ...
  if (length(village$treatcol) > 0) {
    ...
    df_ppck_stats <- df_ppck |>
      dplyr::group_by(
        !!rlang::sym(village$treatcol),
        time,
        replicate,
        donorid,
        donor
      ...
      dplyr::summarise(
        mean = mean(value),
        median = median(value),
        low_ci = quantile(value, probs = lowci),
        high_ci = quantile(value, probs = highci),
        .groups = "drop"
      )
    ...
    df_ppck_stats <- df_ppck |>
      dplyr::group_by(time, replicate, donorid, donor) |>
      dplyr::summarise(
        mean = mean(value),
        median = median(value),
        low_ci = quantile(value, probs = lowci),
  ...
  mergecols <- colnames(df_ppckpt)[
    colnames(df_ppckpt) %in% colnames(df_ppck_stats)
  ]
  ...
  df_ppckpt$ppck <- ifelse(
    df_ppckpt$representation < df_ppckpt$high_ci &
  ...
  if (isFALSE(village$sim)) {
    ...
    if (length(village$treatcol) == 1) {
      ...
      df_ppck_stats <- df_ppck |>
        dplyr::group_by(time, donorid, donor) |>
        dplyr::summarise(
          mean = mean(value),
          median = median(value),
          low_ci = quantile(value, probs = lowci),
    ...
    if (length(village$treatcol) == 1) {
      ...
      df_ppck_stats <- df_ppck_stats |>
        dplyr::rename(Dose = !!rlang::sym(village$treatcol))
      ...
      p <- ggplot(df_ppck_stats) +
        ...
        geom_errorbar(
          aes(y = time, xmin = low_ci, xmax = high_ci),
          width = 0,
          lwd = 0.5
        ) +
        geom_point(
          data = df_ppckpt,
          aes(x = representation, y = time, color = color),
          color = df_ppckpt$color,
          shape = "|"
        ) +
        geom_point(
          data = df_ppckpt[df_ppckpt$point == 23, ],
          aes(x = high_ci + 0.05, y = time),
          color = "darkred",
          size = 2,
          shape = 4
        ) +
    ...
    ggsave(
      paste0(village$outdir, village$name, "_ppck_barplot.png"),
      bg = "white",
      width = pltwidth,
      height = pltheight,
      units = "in"
...
plot_ESS.Village <- function(village) {
  ...
  ESS <- village$summary_params[
    !startsWith(village$summary_params$paramname, "beta_raw") &
  ...
  ggsave(
    paste0(village$outdir, village$name, "_ESS.png"),
    width = pltwidth,
    height = 4,
    units = "in"
  )
...
plot_rawdata.Village <- function(village) {
  ...
  ggsave(
    paste0(village$outdir, village$name, "_rawdata_scatter.png"),
    bg = "white",
    width = plt_width,
    height = plt_height,
    units = "in"
  ...
  data <- data |>
    dplyr::group_by(Dose) |>
    dplyr::mutate(diff = min(replicate) - 1) |>
    dplyr::ungroup()

  data$replicate <- data$replicate - data$diff

  data <- data |>
    dplyr::rename(!!rlang::sym(timeunit) := time)
  ...
  ggsave(
    paste0(village$outdir, village$name, "_rawdata_stackbar.png"),
    bg = "transparent",
    width = plt_width,
    height = plt_height,
    units = "in"
...
plot_donorcov.Village <- function(village, lowci, highci) {
  ...
  df <- rstan::extract(
    ...
    pars = village$summary_params$paramname[
      startsWith(village$summary_params$paramname, "tau_g[")
    ],
    permuted = FALSE
  ) |>
    as.data.frame()
  ...
  df$cov <- unlist(village$donorcov[df$cov.id]) |>
    as.factor()
  ...
  ggsave(
    paste0(village$outdir, village$name, "_regressors.png"),
    bg = "white",
    width = pltwidth,
    height = 4,
    units = "in"
...
plot_overdispersion.Village <- function(village, lowci, highci) {
  ...
  df <- rstan::extract(
    village$fit,
    pars = village$summary_params$paramname[
      startsWith(village$summary_params$paramname, "theta")
    ],
    permuted = FALSE
  ) |>
    as.data.frame()
  ...
  df$low_ci <- quantile(df$value, probs = lowci)
  df$high_ci <- quantile(df$value, probs = highci)
  ...
  ggsave(
    paste0(village$outdir, village$name, "_theta.png"),
    bg = "white",
    width = 1.5,
    height = 4,
    units = "in"
  ...
  df <- rstan::extract(
    village$fit,
    pars = village$summary_params$paramname[
      startsWith(village$summary_params$paramname, "phi_r")
    ],
    permuted = FALSE
  ) |>
    as.data.frame()
  ...
  df <- df |>
    ...
    dplyr::mutate(
      mean = mean(value),
      low_ci = quantile(value, probs = lowci),
      high_ci = quantile(df$value, probs = highci),
      id = "phi_r"
    )

  df_phi <- rstan::extract(
    village$fit,
    pars = "phi",
    permuted = FALSE
  ) |>
  ...
  df_phi <- df_phi |>
    dplyr::mutate(
      mean = mean(df_phi$value),
      low_ci = quantile(df_phi$value, probs = lowci),
      high_ci = quantile(df_phi$value, probs = highci),
      replicate = NA,
      id = "phi"
    )
  ...
  ggsave(
    paste0(village$outdir, village$name, "_phi_r.png"),
    bg = "white",
    width = village$num_reps * 0.5,
    height = 4,
    units = "in"
  )
}
...
#' @param returnboth If TRUE returns list of data and plots. Data includes lfsr calculations.
...
plot_growthmetric.Village <- function(village, lowci, highci, df_treat = NULL, df_cont, df_donor, df_dcov, returnboth = FALSE) {
  ...
  if (!is.null(df_treat)) {
    ...
    df_treat$chain <- rep(
      1:village$mcmc_chains,
      times = (village$mcmc_samples - village$mcmc_warmup) * village$num_donors
    )

    df_treat <- df_treat |>
      dplyr::group_by(donorid, chain) |>
      dplyr::mutate(iter = dplyr::row_number()) |>
      dplyr::ungroup()
    ...
    if (village$num_donorcov > 0) {
      ...
      df_lfsr <- df_treat |>
        dplyr::distinct(iter, dplyr::across(all_of(village$donorcov))) |>
        dplyr::summarise(
          dplyr::across(
            all_of(village$donorcov),
            ~ min(mean(. >= 0), mean(. <= 0)),
            .names = "lfsr_{.col}"
          )
        )
      ...
      interaction_term <- village$predictors[
        stringr::str_detect(village$predictors, ":.*treatment_|treatment_.*:")
      ]

      if (length(interaction_term) > 0) {
        remove_term <- stringr::str_extract(interaction_term, "(?<!treatment_)\\w+(?=:)|(?<=:)(?!treatment_)\\w+")
        dinteract <- interaction_term |>
          stringr::str_remove(remove_term) |>
          stringr::str_remove(":")
      }
    }
    ...
    df_lfsr_treat <- df_treat |>
      dplyr::group_by(donor, donorid) |>
      dplyr::summarise(lfsr = min(mean(value >= 0), mean(value <= 0)),.groups = "drop")
    ...
    for (d in 1:village$num_doses) {
      ...
      if (village$num_donorcov > 0) {
        for (dc in 1:length(dcov)) {
          df_treat[, as.character(doses[d])] <-
            df_treat[, as.character(doses[d])] +
            df_treat[, dcov[dc]] * df_treat[, paste0(village$donorcols[dc], "_pred")]
        }

        if (length(interaction_term) > 0) {
          df_treat[, as.character(doses[d])] <-
            df_treat[, as.character(doses[d])] +
            df_treat[, dcov[str_detect(dcov, dinteract)]] *
            df_treat[, paste0(interaction_term, "_pred")] *
            doses_scaled[d]
        }
      }
    }
    ...
    df_treat <- df_treat |>
      tidyr::pivot_longer(
        cols = as.character(village$doses),
        names_to = "Dose",
    ...
    df_beta_stats <- df_treat |>
      dplyr::group_by(donor, donorid, Treatment, Dose) |>
      dplyr::summarise(
        mean = mean(growthrate),
        median = median(growthrate),
        low_ci = quantile(growthrate, probs = lowci),
        high_ci = quantile(growthrate, probs = highci),
        .groups = "drop"
      )
    ...
    p <- ggplot(df_treat) +
      ...
      geom_errorbar(
        data = df_beta_stats,
        aes(x = donor, ymin = low_ci, ymax = high_ci),
        width = 0,
        lwd = 0.4,
        color = "black"
      ) +
    ...
    if (length(village$predictors) > 0) {
      ...
      df_lfsr <- df_cont |>
        dplyr::distinct(iter, dplyr::across(all_of(village$predictors))) |>
        dplyr::summarise(
          dplyr::across(
            all_of(village$predictors),
            ~ min(mean(. >= 0), mean(. < 0)),
            .names = "lfsr_{.col}"
          )
        )
    ...
    if (length(village$predictors) > 0) {
      ...
      for (p in 1:length(village$predictors)) {
        df_cont$growthrate <- df_cont$growthrate +
          df_cont[, designcols[p]] * df_cont[, predcols[p]]
      }
    }

    df_beta_stats <- df_cont |>
      dplyr::group_by(donor, donorid) |>
      dplyr::summarise(
        mean = mean(growthrate),
        median = median(growthrate),
  ...
  ggsave(
    paste0(village$outdir, village$name, "_growthrates.png"),
    plot = p,
    bg = "white",
    width = pltwidth,
    height = pltheight,
...
#' @param lowci Lower bound of credible interval
#' @param highci Higher bound of credible interval
#' @param df_all Optional data frame containing treatment parameter draws.
#' @param df_donor Data frame with distinct donor ids/names.
#' @return None
#' @keywords internal
...
plot_eta.Village <- function(village, lowci, highci, df_all, df_donor) {
  ...
  donor_order <- df_beta_stats$donor[df_beta_stats$name == "beta"]
  donor_order <- donor_order[order(df_beta_stats$mean[df_beta_stats$name == "beta"])]
  ...
  df_beta_stats$point[
    (df_beta_stats$low_ci < 0 & df_beta_stats$high_ci > 0) | df_beta_stats$name == "beta"
  ] <- 16
  ...
  ggsave(
    paste0(village$outdir, village$name, "_eta_params.png"),
    bg = "white",
    width = pltwidth,
    height = 4,
    units = "in"
